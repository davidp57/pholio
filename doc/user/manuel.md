...
poetry run pholio --host 0.0.0.0  # exposer sur le réseau local
```

### Exécutable autonome (Windows)

Si vous disposez de `dist/pholio.exe`, double-cliquez dessus ou lancez-le depuis un terminal. Aucune installation Python requise.
...
1. **Panneau gauche** — Configuration (album, format de page, type de mise en page, ordre des photos, couverture)
...
Dans la section **Couverture** du panneau gauche :

1. Cliquez sur **Choisir** et sélectionnez une photo dans la liste
2. Saisissez un titre (optionnel)
3. Relancez la mise en page — la couverture apparaît sur la page 0 (pleine page)

Pour retirer la couverture, cliquez sur **Supprimer** dans la section Couverture.
...
### Redimensionner sans verrouiller la position

Pour changer la taille d'une photo sans la fixer sur une page précise, utilisez le champ **Taille** dans la barre de sélection (voir ci-dessous). Cela modifie uniquement les dimensions sans verrouiller la position dans la mise en page.
...
Utilisez les contrôles de zoom dans la barre d'outils du canvas :

- **Curseur** : ajustez le zoom de 40 % à 200 %
- **Boutons +/−** : incréments de 10 %
- **Reset** : revenir à 100 %
- **Ctrl + molette** : zoom avec la souris sur le canvas
